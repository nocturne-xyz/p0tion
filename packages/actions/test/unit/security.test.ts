...
describe("Security", () => {
    ...
    if (envType === TestingEnvironment.PRODUCTION) {
        describe("GitHub anti-sybil", () => {
            it.skip("should return true for a user that passes the checks", async () => {
                expect(await githubReputation("93448202", 5, 1, 2)).to.be.true
            })
            it.skip("should return false for a user that fails the checks", async () => {
                expect(await githubReputation("121107909", 5, 1, 1)).to.be.false
            })
            it.skip("should not be rate limited when using a personal access token", async () => {
    ...
    describe("GeneratePreSignedURL", () => {
        ...
        it.skip("should throw when given a bucket name that is not used for a ceremony", async () => {
        ...
        if (envType === TestingEnvironment.PRODUCTION) {
            ...
            it.skip("should return a pre-signed URL when given the bucket name for a ceremony", async () => {
        ...
        it.skip("should throw when called without being authenticated", async () => {
    ...
    describe("Security rules", () => {
        ...
        it.skip("should allow a user to retrieve their own data from the firestore db", async () => {
            // login as user1
        ...
        it.skip("should allow any authenticated user to query the ceremony collection", async () => {
            // login as user2
        ...
        if (envType === TestingEnvironment.PRODUCTION) {
            ...
            it.skip("should throw an error if a coordinator tries to read another user's document", async () => {
            ...
            /// @note security rules provided with this project prevent access to other users data
            it.skip("should throw an error if an authenticated user tries to read another user's data", async () => {
                // login as user2
                await signInWithEmailAndPassword(userAuth, users[1].data.email, passwords[1])
                await expect(
                    getDocumentById(userFirestore, commonTerms.collections.users.name, users[0].uid)
            ...
            it.skip("should prevent unauthenticated users from accessing the users collection", async () => {
                await expect(
                    getDocumentById(userFirestore, commonTerms.collections.users.name, users[0].uid)
                ).to.be.rejectedWith("Missing or insufficient permissions.")
            })
            /// @note unauthenticated users should be allowed to access ceremonies data
            it.skip("should allow unauthenticated users from accessing the ceremonies collection", async () => {
                await expect(
    ...
    if (envType === TestingEnvironment.PRODUCTION) {
        ...
        describe("Multipart upload", () => {
            ...
            it.skip("should succeed when the user is the current contributor and is upload valid zkey index file", async () => {
            ...
            it.skip("should revert when the user is not a contributor for this ceremony circuit", async () => {
                // await signInWithEmailAndPassword(userAuth, users[0].data.email, passwords[0])
                await expect(
                    openMultiPartUpload(
                        userFunctions,
            ...
            it.skip("should revert when the user is trying to upload a file with the wrong storage path", async () => {
                await adminFirestore
                    .collection(getCircuitsCollectionPath(ceremonyContributor.uid))
                    .doc(circuitsCurrentContributor.uid)
                    .set({
            ...
            it.skip("should fail when the user is trying to upload a file to a bucket not part of a ceremony", async () => {
                await signInWithEmailAndPassword(userAuth, users[0].data.email, passwords[0])
                await expect(
        ...
        describe("Contribution", () => {
            ...
            it.skip("should not take another user's place in the waiting queue", async () => {
                // register 1 user
                await signInWithEmailAndPassword(userAuth, users[0].data.email, passwords[0])
            ...
            it.skip("should not be possible to block the waiting queue", async () => {
                // register 1 user
                await signInWithEmailAndPassword(userAuth, users[0].data.email, passwords[0])
    ...
    describe("Setup", () => {
        ...
        it.skip("should not be possible to call privileged functions related to setup when not authenticated", async () => {
        ...
        it.skip("should not be possible to call privileged functions related to setup when authenticated as a user without coordinator privileges", async () => {
        ...
        it.skip("should html encode malicious characters passed as part of a ceremony creation data", async () => {
            const ceremonyData = {
        ...
        it.skip("should not html encode safe characters passed as part of a ceremony creation data", async () => {
            const ceremonyData = {
                title: '"><script>alert(1)</script>',
        ...
        /// @note
        it.skip("should not be possible to DoS the system by creating a ceremony with a large amount of data inside", async () => {
            const ceremonyData = {
    ...
    describe("Finalize", () => {
        ...
        it.skip("should not allow a contributor to call finalize specific functions", async () => {
            await signInWithEmailAndPassword(userAuth, users[1].data.email, passwords[1])
        ...
        it.skip("should not allow unauthenticated users to call finalize specific functions", async () => {
            await signOut(userAuth)
            await expect(checkAndPrepareCoordinatorForFinalization(userFunctions, ceremonyClosed.uid)).to.be.rejected
            await expect(
                finalizeCircuit(userFunctions, ceremonyClosed.uid, finalizationCircuit.uid, bucketName, `handle-id`)
    ...
    describe("Authentication", () => {
        ...
        it.skip("should not let anyone authenticate with the wrong password", async () => {
        ...
        it.skip("getCurrentFirebaseAuthUser should retun the current authenticated user and not another user's data", async () => {
            // login as user1
            await signInWithEmailAndPassword(userAuth, users[0].data.email, passwords[0])
        ...
        /// when they are not
        it.skip("should not set a user as coordinator when they are not", async () => {
            // login as user1
            await signInWithEmailAndPassword(userAuth, users[0].data.email, passwords[0])
            const currentAuthenticatedUser = getCurrentFirebaseAuthUser(userApp)
        ...
        it.skip("should not allow disabled accounts to login (email/password auth)", async () => {
            // Disable user.
        ...
        it.skip("should correctly logout an user when calling signOut", async () => {
            await signOut(userAuth)
            expect(() => getCurrentFirebaseAuthUser(userApp)).to.throw(
                "Unable to find the user currently authenticated with Firebase. Verify that the Firebase application is properly configured and repeat user authentication before trying again."
            )
        ...
        if (envType === TestingEnvironment.PRODUCTION) {
            ...
            it.skip("should not allow to authenticate anynomously to Firebase", async () => {
                const auth = getAuth()
            ...
            it.skip("should prevent authentication with the wrong OAuth2 token", async () => {
            ...
            it.skip("should lock out an account after a large number of failed attempts", async () => {
                let err: any
                for (let i = 0; i < 1000; i++) {
                    try {
